        ...
        E: Edge,
{
    ...
    fn vertices_mut(&self) -> VerticesMut<V>;
...
/// Vertices
pub struct Vertices<'a, V> where V: Vertex {
    iter: slice::Iter<'a, V>
}
...
impl<'a, V> Iterator for Vertices<'a, V> where V: Vertex {
    type Item = &'a V;
    fn next(&mut self) -> Option<Self::Item> {
        self.iter.next()
    }
...
impl<'a, V> Iterator for VerticesMut<'a, V> where V: Vertex {
    type Item = &'a mut V;
    fn next(&mut self) -> Option<Self::Item> {
        self.iter.next()
    }
